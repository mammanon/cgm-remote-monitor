...
function init (env, ctx) {
  ...
  function run ( ) {
    // The explicit date filter matters: without one, the query layer quietly
    // limits results to the last four days and the backup silently loses
    // everything older.
    var everything = { count: 100000, find: { created_at: { $gte: '2000-01-01' } } };
    ctx.treatments.list(everything, function (err, treatments) {
      if (err || !treatments) {
        console.error('Backup could not read the treatments', err);
        return;
      }
      ctx.mealcomments.list({ }, function (cErr, comments) {
        if (cErr || !comments) { comments = [ ]; }
        try {
        ...
        copyPhotos(imageIdsIn(treatments), function (copied) {
          prune( );
          console.info('Backup written:', treatments.length, 'treatments,'
            , comments.length, 'comments,', copied, 'new photos');
        });
      });
    });
  }
