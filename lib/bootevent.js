...
function boot (env) {
  ...
  function setupInternals (ctx, next) {
    ...
    ctx.mealimages = require('./mealimages')(env, ctx);
    ctx.mealcomments = require('./mealcomments')(env, ctx);
    ...
    ctx.autobackup = require('./autobackup')(env, ctx);
  ...
  function ensureIndexes (ctx, next) {
    ...
    ctx.store.ensureIndexes(ctx.mealcomments( ), ctx.mealcomments.indexedFields);
